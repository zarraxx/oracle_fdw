...
CREATE EXTENSION oracle_jni_fdw;

-- TWO_TASK or ORACLE_HOME and ORACLE_SID must be set in the server's environment for this to work
CREATE SERVER oracle FOREIGN DATA WRAPPER oracle_jni_fdw OPTIONS (dbserver '', isolation_level 'read_committed', nchar 'true', set_timezone 'true');
...
DO
$$BEGIN
   SELECT oracle_jni_execute('oracle', 'DROP VIEW scott.ttv');
EXCEPTION
   WHEN OTHERS THEN
      NULL;
END;$$;
...
DO
$$BEGIN
   SELECT oracle_jni_execute('oracle', 'DROP TABLE scott.typetest1 PURGE');
EXCEPTION
   WHEN OTHERS THEN
      NULL;
END;$$;
...
DO
$$BEGIN
   SELECT oracle_jni_execute('oracle', 'DROP MATERIALIZED VIEW scott.mattest2');
EXCEPTION
   WHEN OTHERS THEN
      NULL;
END;$$;
...
DO
$$BEGIN
   SELECT oracle_jni_execute('oracle', 'DROP TABLE scott.typetest2 PURGE');
EXCEPTION
   WHEN OTHERS THEN
      NULL;
END;$$;
...
DO
$$BEGIN
   SELECT oracle_jni_execute('oracle', 'DROP TABLE scott.gis PURGE');
EXCEPTION
   WHEN OTHERS THEN
      NULL;
END;$$;

SELECT oracle_jni_execute(
          'oracle',
...
SELECT oracle_jni_close_connections();
...
SELECT oracle_jni_execute('oracle', 'UPDATE typetest1 SET b = 2 WHERE id = 1');
...
666	c枚py	variation	dynamo	眉nicode	Not very long	n'Clobber	DEADF00D	9a0cf1eb-02e2-4b1f-bbe0-449fa4a99969	\\x01020304	\\xFFFF	\N	0.11111	0.43211	0.01010	2100-01-29	2050-04-01 19:30:00	12 hours	0 years
777	fdjkl	r89809rew	^脽[]#~	\N	Das also ist des Pudels Kern.	Foo	00	fe288446-05f6-4074-9e9e-6ee41af7b377	\\x00	\\x00	FALSE	10	1002	1003	2019-05-01	2019-05-01 0:00:00	0 seconds	1 year
\.
ROLLBACK;
